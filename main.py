...
def main():
    parser = argparse.ArgumentParser(
        description="VECM Modeling: Heating Oil vs Crude Oil"
    )
    ...
    for col, results in stationarity.items():
        logging.info(f"{col}:")
        logging.info(
            f"  Level - ADF: {results['level']['statistic']:.4f}, p={results['level']['pvalue']:.4f}"
        )
        logging.info(
            f"  Diff - ADF: {results['diff']['statistic']:.4f}, p={results['diff']['pvalue']:.4f}"
        )
