...
def fetch_data(symbols: list, start: str='2015-01-01', end: str='2024-12-31') -> pd.DataFrame:
...
def test_stationarity(df: pd.DataFrame) -> Dict:
    ...
    for col in df.columns:
        ...
        results[col] = {'level': {'statistic': result[0], 'pvalue': result[1]}, 'diff': {'statistic': result_diff[0], 'pvalue': result_diff[1]}}
    return results

def test_cointegration(df: pd.DataFrame) -> Dict:
    """Perform Johansen cointegration test."""
    result = coint_johansen(df, det_order=0, k_ar_diff=1)
    return {'trace_statistic': result.lr1, 'critical_values': result.cvt, 'eigenvalues': result.eig}
...
def plot_price_relationship(df: pd.DataFrame, forecast: pd.DataFrame, title: str, output_path: Path, plot: bool=False):
    ...
    if forecast is not None:
        ax.plot(forecast.index, forecast['HeatingOil'], linestyle='--', color='#4A90A4', linewidth=1.2, alpha=0.7, label='Forecast (Heating Oil)')
        ax.plot(forecast.index, forecast['CrudeOil'], linestyle='--', color='#D4A574', linewidth=1.2, alpha=0.7, label='Forecast (Crude Oil)')
    ax.set_xlabel('Date')
    ax.set_ylabel('Price')
    ax.legend(loc='best')
